...
class Movie extends CI_Controller{
	...
	public function add_edit_movies(){
		...
		$movie_id = $add_edit == "add_movie" ? "" : $data['common_id'];
		...
		$where = array('title'=>$data['title'],'type' =>1);
		$where_id_in = $add_edit == "add_movie" ? "" : array('common_id'=>$movie_id);
		$where_movie = $add_edit == "add_movie" ? "" : array('common_id' => $movie_id);
		$where_common = $add_edit == "add_movie" ? "" : array('common_id' => $data['common_id']);
			...
			if(empty($this->basic->exist_or_not($where,$where_id_in,"common_master"))){
				...
				$movie_banner = $return_id.'_movie_banner.jpg';
				$movie_poster = $return_id.'_movie_poster.jpg';

				$final_array= array();
				$array_1 = array($movie_banner,'movie_banner');   //comba banner image name to store in database and html input name 
				$array_2 = array($movie_poster,'movie_poster');   //comba banner image name to store in database and html input name 
			...
			$movie_id =''.$item->movie_id .'';		
				...
				$row[] = 	'<div class="flex align-items-center list-user-action">
                              	...
                              	<a class="iq-bg-success" data-toggle="tooltip" data-placement="top" title=""
                                 	data-original-title="Edit" href='.base_url("admin/movie/edit_movie/".$item->common_id."").' ><i
